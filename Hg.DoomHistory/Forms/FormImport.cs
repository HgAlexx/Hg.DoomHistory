        ...
        private void buttonBackupBrowse_Click(object sender, EventArgs e)
        {
            if (folderBrowserDialog.ShowDialog(this) == DialogResult.OK)
            {
                if (Directory.Exists(folderBrowserDialog.SelectedPath))
                {
                    ...
                    if (_backupFolder.Contains(_importFolder) || _importFolder.Contains(_backupFolder))
                    {
                        _importFolder = "";
                        MessageBox.Show(@"The import folder and the backup folder must not contain each other!",
                            @"Nop!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
            ...
            try
            {
                ...
                foreach (FileInfo fileInfo in directoryInfo.GetFiles())
                {
                    ...
                    if (fileInfo.Name == "game.details")
                    {
                        ...
                        if (id > 0)
                        {
                            GameDetails gameDetails = new GameDetails(content);
